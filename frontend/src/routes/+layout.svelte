...
<div class="flex min-h-screen flex-col bg-slate-50 font-sans text-slate-900 selection:bg-blue-500/20 dark:bg-slate-950 dark:text-slate-100">
	<div class="pointer-events-none fixed inset-0 z-0 opacity-[0.03] dark:opacity-[0.07]"
	...
	<nav class="sticky top-0 z-50 w-full border-b border-slate-200/60 bg-white/70 backdrop-blur-xl dark:border-slate-800/60 dark:bg-slate-950/70">
		<div class="mx-auto flex h-20 max-w-5xl items-center justify-between px-4 sm:px-6">
			<a href={resolve('/')} class="group flex items-center gap-3 outline-none">
				<div class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white shadow-lg shadow-blue-500/20 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3">
			...
			<div class="flex items-center gap-6">
				...
				{#if userState.isLoggedIn()}
					<div class="flex items-center gap-4">
						<div class="flex items-center gap-3 rounded-full border border-slate-200 bg-white p-1 pr-4 shadow-sm transition hover:border-blue-300 dark:border-slate-800 dark:bg-slate-900">
				...
				{:else}
					<nav class="flex items-center gap-4">
						<a href={resolve('/login')} class="text-sm font-bold text-slate-600 transition hover:text-blue-600 dark:text-slate-400 dark:hover:text-white">
							登入
						</a>
						<a href={resolve('/register')} class="rounded-xl bg-blue-600 px-6 py-2 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition-all hover:bg-blue-700 hover:-translate-y-0.5">
							註冊
						</a>
					</nav>
				{/if}
	...
	<footer class="border-t border-slate-200/60 bg-white py-12 dark:border-slate-800/60 dark:bg-slate-950">
		<div class="mx-auto max-w-5xl px-4 text-center sm:px-6">
			<p class="text-sm font-medium text-slate-500 dark:text-slate-400">
				&copy; {new Date().getFullYear()} DevBlog. 採用 SvelteKit 5 與 Tailwind CSS 4 構建。
			</p>
		</div>
	</footer>
</div>